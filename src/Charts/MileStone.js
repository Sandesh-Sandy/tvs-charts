...
const MilestoneChart = () => {
  ...
  useEffect(() => {
    ...
    const data = [
      ...
      { date: 'May', plan: 12, revisedPlan: 6, lastYear: 11 },
      ...
      { date: 'Sep', plan: 12, revisedPlan: 6, lastYear: 11 },
      { date: 'Oct', plan: 7, revisedPlan: 3, lastYear: 4 },
      ...
      { date: 'Feb', plan: 9, revisedPlan: 7, lastYear: 6 },
    ...
    const seriesConfig = [
      ...
      { key: 'revisedPlan', color: '#FF0000', symbol: 'square', label: 'Revised Plan' },
      { key: 'lastYear', color: '#008000', symbol: 'diamond', label: 'Last Year Actuals' }
    ];
    ...
    seriesConfig.forEach(series => {
      ...
      // Add points
      svg.selectAll(`point-${series.key}`)
        .data(seriesData)
        .enter()
        .append('path')
        .attr('transform', d => `translate(${x(d.date) + x.bandwidth() / 2},${y(d.value)})`)
        .attr('d', d3.symbol().type(
          series.symbol === 'circle' ? d3.symbolCircle :
          series.symbol === 'square' ? d3.symbolSquare :
          d3.symbolDiamond
        ).size(100))
